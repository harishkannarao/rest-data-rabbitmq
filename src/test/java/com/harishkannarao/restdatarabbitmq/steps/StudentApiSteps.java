...
public class StudentApiSteps extends AbstractBaseSteps {
    ...
    public void iPerformStudentApiGetByIdWithIDOfStudent(String canonicalStudentId) {
        ...
        EntityExchangeResult<String> response = webTestClient().get()
                .uri(applicationUrl() + "/student/{id}", student.getId())
                .exchange()
    ...
    public void iPerformStudentApiGetByIdWithARandomId() {
        EntityExchangeResult<String> response = webTestClient().get()
                .uri(applicationUrl() + "/student/{id}", UUID.randomUUID())
                .exchange()
                .expectBody(String.class)
                .returnResult();
        studentApiHolder.setGetByIdResponse(response);
    ...
    public void iGetANot_foundResponseFromStudentApiGetById() {
        assertThat(studentApiHolder.getGetByIdResponse().getStatus()).isEqualTo(HttpStatus.NOT_FOUND);
    }

    @Then("I get a bad_request response from student-api-get-by-id")
    public void iGetABadRequestResponseFromStudentApiGetById() {
        assertThat(studentApiHolder.getGetByIdResponse().getStatus()).isEqualTo(HttpStatus.BAD_REQUEST);
    }

    @Then("I get a success response from student-api-get-by-id")
    public void iGetASuccessResponseFromStudentApiGetById() {
        assertThat(studentApiHolder.getGetByIdResponse().getStatus()).isEqualTo(HttpStatus.OK);
    }
    ...
    private StudentWithCourseResponseDto extractStudentWithCourseResponseDto() {
        return jsonConverter().fromJson(studentApiHolder.getGetByIdResponse().getResponseBody(), StudentWithCourseResponseDto.class);
    }
}
